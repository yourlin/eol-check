...
from eol_check.utils.logger import configure_logger, debug, error, info
    ...
    with tab2:
        ...
        with col1:
            ...
            if st.session_state.cache_data:
                ...
                # Split data into found and not found
                # Remove this split since we're now handling all data in one table
                # found_data = [item for item in st.session_state.cache_data if item["content"] != "Empty (Not Found)"]
                # not_found_data = [item for item in st.session_state.cache_data if item["content"] == "Empty (Not Found)"]
...
def main():
    ...
    try:
        ...
        print(
            "If you're seeing this error, try running directly with: streamlit run /path/to/eol_check/ui.py"
        )
        sys.exit(1)


if __name__ == "__main__":
    # 如果直接运行此脚本，则执行 run_ui()
    run_ui()
else:
    # 如果作为模块导入，则提供 main() 函数供 CLI 调用
    pass
